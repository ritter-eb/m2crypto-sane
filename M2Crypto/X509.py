...
from M2Crypto import ASN1, BIO, EVP, Err, m2  # noqa
from typing import AnyStr, Optional  # noqa
ptr = bytes  # weird construct working as a C pointer
...
class X509_Extension:  # noqa
    ...
    def get_name(self):
        ...
        """
        return str(m2.x509_extension_get_name(self.x509_ext))
...
class X509_Name:  # noqa
    ...
    def __getattr__(self, attr):
        ...
        if attr in self.nid:
            assert m2.x509_name_type_check(self.x509_name), \
                "'x509_name' type error"
            return str(m2.x509_name_by_nid(self.x509_name, self.nid[attr]))
...
class X509:
    ...
    def get_fingerprint(self, md='md5'):
        ...
        md.update(der)
        digest = md.final()
        return str(binascii.hexlify(digest).upper())
